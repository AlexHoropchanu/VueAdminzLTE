              ...
              <b-tabs card>
                ...
                >
                  <b-card-text>
                    ...
                    <b-form-group label="Логотип" :label-for="'logo-' + key">
                      <b-form-file
                        class="col-lg-6 col-md-8 col-12"
                        accept="image/*"
...
export default {
  ...
  async mounted() {
    ...
    if ((await getData(this.path)) != null) {
      this.hallsFull = await getData(this.path);
    } else {
  ...
  methods: {
    ...
    galleryPreview(index, lang, event) {
      ...
      this.$set(this.hallsFull[lang].gallery, index, URL.createObjectURL(file));
    ...
    async save() {
      ...
      this.loadingAll = 0;
      this.loadingLeft = 0;
      await saveDb(`/halls/${this.$route.params.id}`, this.hallsFull);
      this.saved = true;
      this.loadingState = "";
      ...
      setTimeout(() => {
        this.$router.push({
          name: "CardCinema",
          params: { id: this.hallsFull.cinema },
        });
      }, 2000);
    },
    ...
    async writeBannerToStorage() {
      for (const lang in this.bannerWrite) {
        ...
        if (el != "") {
          try {
            this.hallsFull[lang].banner = await saveStorage(
              el,
              "halls",
    ...
    async writeLogoToStorage() {
      for (const lang in this.logoWrite) {
        ...
        if (el != "") {
          try {
            this.hallsFull[lang].logo = await saveStorage(
              el,
              "halls",
              this.$route.params.id
            );
    ...
    async writeGalleryToStorage() {
      for (const language in this.galleryWrite) {
        for (let i = this.galleryWrite[language].length - 1; i >= 0; i--) {
          ...
          if (el != "") {
            try {
              this.hallsFull[language].gallery[i] = await saveStorage(
                el,
                "halls",
                this.$route.params.id
              );
